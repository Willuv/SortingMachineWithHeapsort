...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //ASK JEREMY IF ITS OK TO TAKE THIS DIRECTLY FROM THE LAB TEMPLATE
        if (i != j) {
            T tmp = array[i];
            array[i] = array[j];
            array[j] = tmp;
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (last - top > 0) {
            int childIndex = top * 2 + 1;
            if (childIndex + 1 <= last && order.compare(array[childIndex],
                    array[childIndex + 1]) > 0) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //jake's
        if (!isHeap(array, 0, array.length - 1, order)) {
            //top is same as last at first
            siftDown(array, top, array.length - 1, order);
            if (!isHeap(array, 0, array.length - 1, order) && top - 1 >= 0) {
                heapify(array, top - 1, order);
            }
        }
